...
<template>
    ...
    <h2 class="text-xl text-slate-50 font-semibold pl-4 mt-8">Activités à proximité</h2>
        <section class="mx-2 mt-4 text-slate-50 max-w-full overflow-x-scroll flex relative">
            <div v-for="activitiy in activitiesByDist" class=" w-1/2 h-72 ml-2 relative">    
                <article class="bg-myblue-searchbar w-full h-full rounded-2xl shadow-md shrink-0" v-if="activitiy.distance <= rayon">
                        ...
                        <div class="mx-2 mt-4 flex flex-col">
                            ...
                            <p class="text-sm font-light">{{ activitiy.user.name}} 
                                <span>
